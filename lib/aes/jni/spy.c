...
#define MIN_CACHE_MISS_CYCLES (110)           /*x-86*/
#define THRESHOLD (250)                       /*armv7-----Nexus-5*/
#define CPU 0
...
int main()
{
  ...
  /*Mapping the shared library with read and execute permissions*/
 ...
 /*Checking if the library is mapped correctly*/  
 ...
 // unsigned char plaintext[]="Aditya";
  ...
  printf("<1>Number of active cpus %zu\n",number_of_cpus);
  cpu_set_t mask;
  ...
  if( sched_setaffinity( 0, sizeof(mask), &mask ) == -1 )
   {
      printf("WARNING: Could not set CPU Affinity, continuing...\n");}
		  ...
		  printf("<2>The affinity has been set..\n");
	  }
...
/* Probing the T-table elements*/
  for (probe =(base + 0x14f45c); probe < (base + 0x14F75C); probe += 64) // 64: Cache-line size    
      {	
	  int j=0;
	...
	/*  printf("\nThe value of probe's set index is %zu\n",set_index);
      ...
      */
       ...
       for (size_t i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
		...
		/* Warm-up round :1st AES encryption round  */
		
       AES_encrypt(plaintext,ciphertext,&key_struct);
       
		/* 2nd  AES Encryption round */
...
{  
	if((num%12)==0)
    	{	printf("\n");
			//printf("%p\t",(timings[num].map_element));
